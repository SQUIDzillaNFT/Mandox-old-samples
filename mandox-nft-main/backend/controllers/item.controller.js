...
exports.getItemsPerCollection = (req, res) => {
  const ObjectId = require('mongodb').ObjectId;
  var good_id = new ObjectId(req.query.id);
  NFTItem.find({
    collectionId: { $in: good_id },
    status: { $in: 0 }
  }).then((items) => {
    res.send(items);
  }).catch((e) => res.status(500).send({ message: e }));
...
exports.updateItemStar = (req, res) => {
  const ObjectId = require('mongodb').ObjectId;
  ...
  NFTItem.findOne({
    ...
    if (!item.stars) {
        item.stars = [req.body.account];
    } else if (!item.stars.includes(req.body.account)){
      flag = true;
      item.stars.push(req.body.account);
    } else {
      item.stars = item.stars.filter(function(value) {
        return value != req.body.account;
      });
    }
